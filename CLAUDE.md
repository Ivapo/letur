...
frontmatter schema, the phase gate, the review loop. Spec ids are `ltr-NNN`, allocated
as `max(existing) + 1` and never reused. **Four specs here carry `mpdf-NNN` instead** —
`mpdf-003`, `006`, `009` and `010`, which arrived with `mpdf-011` Phase 1 when this
repository split off the engine. They keep their numbers, because an id is never
renumbered and never reused; `.spec-lint.yaml`'s `id_prefix` is the list that says so.

**The observable this project produces is: the page the author sees beside their text,
redrawn as they write it.** Every phase says whether it produces one; a phase that does
not is argued for explicitly.

**The engine is next door.** `md2pdf-core` turns the markdown into the PDF this window
draws, and it lives in `Ivapo/md2pdf` as a dependency by version rather than a directory
beside this one. A change to what the dialect *accepts* is a spec there, not here. What
is here is the window: the two panes, the project it opens, the pages it rasterises.
